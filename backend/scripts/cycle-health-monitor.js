...
class CycleHealthMonitor {
  ...
  async detectAnomalies() {
    ...
    try {
      // Check for cycles created outside normal hours (00:00-02:00 UTC)
      const result = await db.query(`
        SELECT cycle_id, created_at, EXTRACT(HOUR FROM created_at) as hour_created
      ...
      result.rows.forEach(cycle => {
        anomalies.push({
          ...
          message: `Cycle ${cycle.cycle_id} created at ${cycle.hour_created}:00 UTC (outside normal 00:00-02:00 window)`,
          severity: 'warning'
        });
      });
