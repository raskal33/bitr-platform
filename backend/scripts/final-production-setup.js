...
/**
 * Final Production Analytics Setup
 * Sets up analytics system for production use without clearing existing data
 * Ensures all tables are populated with meaningful data
 */
...
class FinalProductionSetup {
  ...
  async start() {
    ...
    this.isRunning = true;
    console.log('🚀 Final Production Analytics Setup started');
  }

  async stop() {
    this.isRunning = false;
    console.log('🛑 Final Production Analytics Setup stopped');
  }
      ...
      for (const market of sampleMarkets) {
        try {
          ...
          if (error.code === '23505') {
            ...
            console.log(`   ⚠️ Market ${market.fixture_id} already exists, skipping`);
          } else {
            throw error;
          }
        }
...
async function runFinalProductionSetup() {
  console.log('🚀 Starting final production analytics setup...');
  ...
  try {
    ...
    if (isValid) {
      console.log('🎉 Final production analytics setup complete!');
      ...
      console.log('   - Duplicate-safe operations');
    } else {
...
if (require.main === module) {
  runFinalProductionSetup();
}

module.exports = FinalProductionSetup;
