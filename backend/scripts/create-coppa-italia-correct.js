...
/**
 * Create Coppa Italia Market (Correct Odds)
 * 
 ...
 * Odds: 2.00x (minimum working odds)
 * Creator Stake: 100 BITR
 * 
 * This tests the contract with the correct minimum odds.
 */

const { ethers } = require('ethers');
require('dotenv').config();

async function createCoppaItaliaCorrect() {
  console.log('🎯 Creating Coppa Italia Market (Correct Odds)...');
  ...
  try {
    ...
    // Coppa Italia market parameters with correct odds
    const params = {
      predictedOutcome: ethers.encodeBytes32String("YES"), // Udinese wins
      odds: 200, // 2.00x in contract format (minimum working odds)
    ...
    console.log(`Odds: ${params.odds / 100}x (minimum working odds)`);
    ...
    try {
      ...
      if (receipt.status === 1) {
        ...
        console.log('\n📋 Key Learnings:');
        console.log('- Minimum working odds: 2.00x (200 in contract format)');
        console.log('- Market ID: User-defined bytes32 identifier');
        console.log('- Pool ID: Auto-incremented by contract');
        console.log('- Contract validates all parameters before creation');
