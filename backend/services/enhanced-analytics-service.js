...
class EnhancedAnalyticsService {
  ...
  async initializeAnalytics() {
    try {
      ...
      const operations = [
        ...
        { name: 'Pool Analytics', fn: () => this.populatePoolAnalytics() },
  ...
  async populateUserAnalytics() {
    try {
      ...
      // Get all unique users from slips
      const usersResult = await db.query(`
        SELECT DISTINCT player_address as user_address
      ...
      for (const user of usersResult.rows) {
        ...
        const statsResult = await db.query(`
          SELECT 
            COUNT(*) as total_bets,
            COUNT(CASE WHEN is_evaluated = true THEN 1 END) as winning_bets,
            AVG(CASE WHEN is_evaluated = true THEN final_score ELSE 0 END) as avg_score,
            MAX(CASE WHEN is_evaluated = true THEN final_score ELSE 0 END) as max_score,
            COUNT(CASE WHEN is_evaluated = true AND final_score = 10 THEN 1 END) as perfect_scores
          FROM oracle.oddyssey_slips
        ...
        // Calculate reasonable avg_odds (normalize large scores to odds-like values)
        let avgOdds = 1.0;
        if (stats.avg_score && stats.avg_score > 0) {
          // Convert score to odds-like value (score/1000, capped at 999.99)
          avgOdds = Math.min(stats.avg_score / 1000, 999.99);
        }
        ...
        `, [
          userAddress,
          stats.total_bets || 0,
          stats.winning_bets || 0,
          0, // total_staked - will be calculated from pools
          0, // total_won - will be calculated from prizes
          winRate,
          avgOdds
        ]);
      }
    ...
    try {
      ...
      const poolsResult = await db.query(`
        SELECT 
          ...
          CASE WHEN is_evaluated = true THEN evaluated_at ELSE NULL END as event_end_time,
          placed_at + INTERVAL '24 hours' as betting_end_time,
          placed_at as created_at,
          CASE WHEN is_evaluated = true THEN evaluated_at ELSE NULL END as settled_at,
