...
class CycleMonitor {
  constructor() {
    ...
    this.thresholds = {
      ...
      offScheduleCreation: 1, // Alert if any cycles created outside 00:00-02:00 UTC
  ...
  async checkRecentFailures() {
    try {
      ...
      // Only check if we're past the expected cycle creation time (00:04 UTC)
      if (hour >= 1) {
        const result = await db.query(`
