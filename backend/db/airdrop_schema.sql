...
-- Add description column if it doesn't exist (safe operation)
DO $$ 
BEGIN
    -- First ensure the table exists and is not a view
    IF EXISTS (
        ...
        -- Then check if description column exists
        IF NOT EXISTS (
            ...
            BEGIN
                ALTER TABLE airdrop.statistics ADD COLUMN description TEXT;
            EXCEPTION WHEN OTHERS THEN
                -- Ignore any errors when adding the column
                NULL;
            END;
        END IF;
    END IF;
